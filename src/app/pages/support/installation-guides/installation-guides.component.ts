...
@Component({
  ...
  styleUrls: ['./installation-guides.component.scss']
})
export class InstallationGuidesComponent implements OnInit {
  displayedColumns: string[] = ['image', 'fileName', 'downloadUrl' ];
  dataSource: MatTableDataSource<InstallationGuides>;
  ...
  constructor(
    ...
    this.appSettings.settings$.pipe(tap(settings => {
      ...
      this.susiSupportDataService.getOne(this.settings.id).pipe(tap(data => {
        this.dataSource = new MatTableDataSource(data.installationGuides);
