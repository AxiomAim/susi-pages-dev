...
@Component({
  selector: 'app-selection-guides',
  templateUrl: './selection-guides.component.html',
  styleUrls: ['./selection-guides.component.scss']
})
export class SelectionGuidesComponent implements OnInit {
  displayedColumns: string[] = ['image', 'fileName', 'downloadUrl' ];
  dataSource: MatTableDataSource<selectionGuides>;
  ...
  constructor(
    ...
    this.appSettings.settings$.pipe(tap(settings => {
      ...
      this.susiSupportDataService.getOne(this.settings.id).pipe(tap(data => {
        this.dataSource = new MatTableDataSource(data.selectionGuides);
