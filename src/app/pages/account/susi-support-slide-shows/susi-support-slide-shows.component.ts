...
@Component({
  selector: 'app-susi-support-slide-shows',
  templateUrl: './susi-support-slide-shows.component.html',
  styleUrls: ['./susi-support-slide-shows.component.scss']
})
export class SusiSupportSlideShowsComponent implements OnInit {
  public sub: any;
  public supportId: any = 'susiadaptors.com';
  public supportCategory: any = 'slideShows';
  public support: SUSISupportDto = SUSISupportModel.emptyDto();
  public slideShows: any;
  ...
  ngOnInit(): void {
    ...
    this.sub = this.activatedRoute.params.subscribe(params => {
      ...
      this.susiSupportDataService.getOne(this.supportId).pipe(tap(res => {
        this.support = res;
        this.slideShows = this.support.slideShows;
      })).subscribe();
    });
  ...
  remove(slideShows): void {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        this.support.slideShows = this.support.slideShows.filter(obj => obj !== slideShows);
  ...
  drop(event: CdkDragDrop<string[]>) {
    moveItemInArray(this.support.slideShows, event.previousIndex, event.currentIndex);
