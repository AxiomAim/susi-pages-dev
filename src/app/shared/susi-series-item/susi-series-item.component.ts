...
import { Series, Product, Property } from '../../app.models';
...
@Component({
  selector: 'app-susi-series-item',
  templateUrl: './susi-series-item.component.html',
  styleUrls: ['./susi-series-item.component.scss']
})
export class SusiSeriesItemComponent implements OnInit {
  @Input() series: Series;
  ...
  ngOnChanges(changes: {[propKey: string]: SimpleChange}){  
    if(changes.viewColChanged){
      ...
      if(!changes.viewColChanged.isFirstChange()){
        if(this.series.gallery.length > 1){     
  ...
  public onCompare(){
    return this.appService.Data.compareList.filter(item=>item.id == this.series.id)[0];
  }
  ...
  public onFavorites(){
    return this.appService.Data.favorites.filter(item=>item.id == this.series.id)[0];
  }
